...
##########################################################################  
PKGREL = /tmp/roku/packages
ZIPREL = /tmp/roku/zips
SOURCEREL = ..
...
install: $(APPNAME)
	...
	# @curl -s -S -F "mysubmit=Install" -F "archive=@$(ZIPREL)/$(APPNAME).zip" -F "passwd=$(USERPASS)" http://$(ROKU_DEV_TARGET)/plugin_install | grep "<font color" | sed "s/<font color=\"red\">//"

	@if [ $(HTTPSTATUS) -eq 401 ]; \
	then \
		curl --user "rokudev":$(USERPASS) --digest -s -S -F "mysubmit=Install" -F "archive=@$(ZIPREL)/$(APPNAME).zip" -F "passwd=" http://$(ROKU_DEV_TARGET)/plugin_install | grep "<font color" | sed "s/<font color=\"red\">//" | sed "s[</font>[[" ; \
	else \
